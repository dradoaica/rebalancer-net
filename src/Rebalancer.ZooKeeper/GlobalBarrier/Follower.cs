    ...
    public class Follower : Watcher, IFollower
    {
        ...
        public async Task<BecomeFollowerResult> BecomeFollowerAsync()
        {
            try
            {
                ...
                await zooKeeperService.WatchStatusAsync(this);
                logger.Info(clientId, $"Follower - Set a watch on status node");
            }
                    ...
                    switch (followerEvent)
                    {
                        ...
                        case FollowerEvent.RebalancingTriggered:
                            ...
                            else
                            {
                                ...
                                logger.Info(clientId, "Follower - Status change received");
            ...
            try
            {
                ...
                switch (result)
                {
                    case RebalancingResult.Complete:
                        logger.Info(clientId, "Follower - Status change complete");
                        break;

                    case RebalancingResult.Cancelled:
                        logger.Warn(clientId, "Follower - Status change cancelled");
                        break;
        ...
        private async Task<RebalancingResult> ProcessStatusChangeAsync(CancellationToken rebalancingToken)
        {
            ...
            else if (status.RebalancingStatus == RebalancingStatus.ResourcesGranted)
            {
                ...
                {
                    logger.Warn(clientId, "Follower - The resources granted status change has been received while already in the started state. Stopped all activity first");
                    await store.InvokeOnStopActionsAsync(clientId, "Follower");
                }
